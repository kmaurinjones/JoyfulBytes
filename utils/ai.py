...
def create_image_gen_prompt(story_text: str, model: str = "o1-mini-2024-09-12", tqdm_desc: str = None, feedback: dict[str, float] = None):
    ...
    - Characters should have slightly exaggerated features for humor
    ...
    the prompt
    - The image text needs to be less than 4 words, and be simple and short so it can be easily generated by the model. Include the text in [] brackets and quotes, such as ["IMAGE TEXT"].
    - The image text must be overlayed on the image somewhere, so ensure that the prompt includes explicit instructions on how to overlay the image text
    - Write the image text in the prompt in ALL CAPS, separated by a newlines in the prompt to clearly delineate it from the rest of the prompt
